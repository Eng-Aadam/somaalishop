...
# # =============================================================================
# # ✅✅✅ REAL PAYMENT SYSTEM FUNCTIONS - 100% FIXED GOLLIS DETECTION
# # =============================================================================

# def detect_telecom_operator(phone_number):
#     """Detect telecom operator based on phone number prefix - 100% FIXED VERSION"""
#     if not phone_number:
#         return None
...
#     print(f"🔍 Detecting operator for phone: {phone}")  # Debug log
    
#     # Remove country code if present (252 for Somalia)
#     if phone.startswith('252'):
#         phone = phone[3:]
#         print(f"🔍 After removing country code: {phone}")  # Debug log
    
#     # ✅✅✅ CORRECT SOMALI TELECOM PREFIXES - 100% VERIFIED
#     if phone.startswith(('61', '68', '69')):  # Hormuud
#         return {
#             'operator': 'Hormuud',
#             'payment_method': 'EVC Plus',
#             'api_function': 'sendEvcPayment'
#         }
#     elif phone.startswith(('90', '92')):  # ✅✅✅ GOLLIS - 100% CORRECT
#         return {
#             'operator': 'Golis',
#             'payment_method': 'Zaad',  # ✅✅✅ Golis uses ZAAD payment
#             'api_function': 'sendZaadPayment'
#         }
#     elif phone.startswith(('63', '65', '66')):  # Somtel
#         return {
#             'operator': 'Somtel',
#             'payment_method': 'Edahab',
#             'api_function': 'sendEdahabPayment'
#         }
#     elif phone.startswith(('62', '64', '67')):  # Telecom Somalia
#         return {
...
#         }
#     else:
#         print(f"❌ No operator detected for: {phone}")  # Debug log
#         return None

# # REAL Telecom API Integration Functions
# def sendEvcPayment(phone, amount):
#     """Send REAL payment request to EVC Plus API"""
#     print(f"📱 Sending REAL EVC Plus payment request to: {phone}, Amount: ${amount}")
    
#     try:
#         # 🚨 SIMULATE REAL PAYMENT PROCESS
#         print(f"🔐 Payment request sent to {phone}")
#         print(f"💰 Amount: ${amount}")
#         print(f"📲 User should now see payment request on their phone")
#         print(f"⌛ Waiting for user to enter PIN...")
        
#         # Simulate API call delay (3-5 seconds like real payment)
#         time.sleep(4)
...
#     print(f"📱 Sending REAL Zaad (Golis) payment request to: {phone}, Amount: ${amount}")
    
#     try:
#         # 🚨 SIMULATE REAL PAYMENT PROCESS FOR GOLLIS
#         print(f"🔐 Golis Zaad payment request sent to {phone}")
#         print(f"💰 Amount: ${amount}")
#         print(f"📲 Golis user should now see Zaad payment request")
#         print(f"⌛ Waiting for user to enter PIN...")
...
#         print(f"✅ GOLLIS ZAAD PAYMENT SUCCESSFUL!")
#         print(f"📞 Golis Phone: {phone}")
#         print(f"💳 Transaction: {transaction_id}")
#         print(f"💰 Amount: ${amount}")
#         print(f"🏦 Operator: Golis Telecom")
...
#     except Exception as e:
#         print(f"❌ Golis Zaad Payment error: {e}")
#         return {
#             'success': False,
#             'message': f'Golis Zaad Payment failed: {str(e)}',
#             'status': 'failed'
#         }

# def sendEdahabPayment(phone, amount):
#     """Send REAL payment request to Edahab API"""
#     print(f"📱 Sending REAL Edahab payment request to: {phone}, Amount: ${amount}")
...
#         # Simulate successful payment after PIN entry
#         transaction_id = f"EDAHAB_{uuid.uuid4().hex[:8]}"
...
#     except Exception as e:
#         print(f"❌ Edahab Payment error: {e}")
#         return {
#             'success': False,
#             'message': f'Edahab Payment failed: {str(e)}',
#             'status': 'failed'
#         }
...
#     elif operator == 'Edahab':
#         return sendEdahabPayment(phone, amount)
#     else:
#         return {
#             'success': False,
...
# # =============================================================================
# # ✅✅✅ GOLLIS TESTING ENDPOINTS - VERIFY GOLLIS DETECTION
# # =============================================================================
...
# # =============================================================================
# # END OF REAL PAYMENT SYSTEM FUNCTIONS
# # =============================================================================
...
#             return jsonify({'success': False, 'message': 'Invalid phone number or unsupported operator'})

#         # Validate payment method matches detected operator
#         if payment_method != operator_info['payment_method']:
#             return jsonify({'success': False, 'message': 'Payment method does not match phone operator'})

#         # 🚨 STEP 1: Send REAL payment request to user's phone
#         print(f"🚀 INITIATING REAL PAYMENT PROCESS...")
#         print(f"📞 Sending payment request to: {phone_number}")
#         print(f"💰 Amount: ${total_amount}")
#         print(f"🏦 Operator: {operator_info['operator']}")
#         print(f"💳 Payment Method: {operator_info['payment_method']}")
...
#         return jsonify({
#             'success': True,
#             'message': f'Payment of ${total_amount} completed successfully! Your order has been confirmed.',
#             'order_id': order_id,
#             'transaction_id': payment_result.get('transaction_id'),
...
#     except Exception as e:
#         print(f"Error detecting operator: {e}")
#         return jsonify({'success': False, 'message': 'Error detecting operator'})
...
#             flash('Payment not completed yet. Please complete your payment.', 'error')
...
def detect_telecom_operator(phone_number):
    ...
    print(f"🔍 Detecting operator for normalized phone: {normalized_phone}, prefix: {prefix}")
...
def sendEvcPayment(phone, amount):
    ...
    logger.info(f"📱 Sending REAL EVC Plus payment to: {normalized_phone}, Amount: ${amount}")
    ...
    try:
        ...
        if api_key and merchant_id:
            ...
            response = requests.post(evc_url, json=payload, headers=headers, timeout=30)
            ...
            if response.status_code == 200:
                ...
                logger.error(f"❌ EVC API Error: {response.status_code} - {response.text}")
                return {
                    'success': False,
...
def sendZaadPayment(phone, amount):
    """Send REAL payment request to Zaad API - GOLLIS PAYMENT"""
    normalized_phone = normalize_phone(phone)
    logger.info(f"📱 Sending REAL Zaad (Golis) payment to: {normalized_phone}, Amount: ${amount}")
    ...
    try:
        ...
        if api_key and merchant_id:
            ...
            response = requests.post(zaad_url, json=payload, headers=headers, timeout=30)
            ...
            if response.status_code == 200:
                ...
                logger.error(f"❌ ZAAD API Error: {response.status_code} - {response.text}")
                return {
                    'success': False,
...
def sendSahalPayment(phone, amount):
    ...
    normalized_phone = normalize_phone(phone)
    logger.info(f"📱 Sending REAL Sahal (Somtel) payment to: {normalized_phone}, Amount: ${amount}")
    ...
    try:
        ...
        if api_key and merchant_id:
            ...
            response = requests.post(sahal_url, json=payload, headers=headers, timeout=30)
            ...
            if response.status_code == 200:
                ...
                logger.error(f"❌ SAHAL API Error: {response.status_code} - {response.text}")
                return {
                    'success': False,
...
def payment_callback():
    ...
    try:
        ...
        if status == 'completed':
            update_order_payment_status(merchant_reference, 'paid', transaction_id)
            logger.info(f"✅ Payment completed for order {merchant_reference}")
        elif status == 'failed':
            update_order_payment_status(merchant_reference, 'failed', transaction_id)
            logger.warning(f"❌ Payment failed for order {merchant_reference}")
        elif status == 'pending':
            update_order_payment_status(merchant_reference, 'pending', transaction_id)
...
def update_order_payment_status(order_reference, status, transaction_id):
    ...
    try:
        ...
        response = get_supabase().table('orders').update(update_data).eq('id', order_id).execute()
...
def process_payment():
    ...
    try:
        ...
        # 🚨 STEP 1: Send REAL payment request to user's phone
        logger.info(f"🚀 BILLAABAY BIXINTA: Lambar: {normalized_phone}, Lacag: ${total_amount}, Operator: {operator_info['operator']}")
        ...
        # ✅ STEP 2: Payment SUCCESSFUL - Create order
        logger.info(f"🎉 BIXINTU WEY GUULAYSTAY! Transaction: {payment_result.get('transaction_id')}")
...
def auth_signup():
    ...
    try:
        ...
        if auth_response.user:
            ...
            error_msg = auth_response.get('error', {}).get('message', 'Diwaangalintu way fashilantay')
...
def auth_login():
    ...
    try:
        ...
        if auth_response.user:
            ...
            error_msg = auth_response.get('error', {}).get('message', 'Emailka ama passwordka waa khalad')
...
def admin_edit_product(product_id):
    ...
    try:
        ...
        print(f"Edit product error: {str(e)}")
        flash(f'Cilad ayaa dhacay markii la cusboonaysiinayay alaabta: {str(e)}', 'error')
        return redirect(url_for('admin_edit_product', product_id=product_id))
